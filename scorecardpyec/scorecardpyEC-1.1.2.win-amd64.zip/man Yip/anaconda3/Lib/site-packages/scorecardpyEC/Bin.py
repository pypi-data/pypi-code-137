...
class ECBin():
    ...
    def x2bin(x,binx,with_label_row_no=True):
        '''
    ...
    def trans_data_to_woebin(dat,temp_bins,y='y',ylabel={'good':0,'bad':1},weight=None):
        dat = dat.copy()
        dat[y] = pd.concat([pd.Series(0,dat[y].loc[dat[y]==ylabel['good']].index),pd.Series(1,dat[y].loc[dat[y]==ylabel['bad']].index)]).loc[dat.index]
        new_bins={}
        def _f1(x):
            x_bin_df = ECBin.x2bin(x,temp_bins[x.name],False)
            g = dat.groupby(x_bin_df)[y].agg(['count','sum','mean']) 
            g.index.name='bin'
            g.reset_index(inplace=True)
            g.rename(columns = {'sum':'bad','mean':'badprob'},inplace=True)
            g['count_distr'] = g['count']/dat.shape[0]
            g['good'] = g['count'] - g['bad']
            g=g[['bin','count','count_distr','good','bad','badprob']]
            all_bad = g.bad.sum()
            all_good = g.good.sum()
            g['woe'] = np.log( (g['bad']/all_bad) / (g['good']/all_good) )
            g['bin_iv'] = ( (g['bad']/all_bad) - (g['good']/all_good) )*g['woe']
            g['total_iv'] = g['bin_iv'].sum()
            g['bin']=g['bin'].apply(str)
            temp_g = temp_bins[x.name]
            g = temp_g.merge(g,on='bin',how='outer')
            g = g[['variable', 'bin', 'count_y', 'count_distr_y', 'good_y', 'bad_y',
               'badprob_y', 'woe_y', 'bin_iv_y', 'total_iv_y', 'breaks',
               'is_special_values']]
            g.columns = ['variable', 'bin', 'count', 'count_distr', 'good', 'bad',
               'badprob', 'woe', 'bin_iv', 'total_iv', 'breaks',
               'is_special_values']
            new_bins[x.name]=g            
        cols = list(temp_bins.keys())
        dat[cols].apply(_f1)
        
        if weight is not None:
            new_bins = ECBin.adj_bin_with_weight(new_bins,dat,y,weight,ylabel)
        return new_bins
    ...
    def woevalue_mp(dats,bins,step=50,no_cores=None):
        ...
        '''
        for k,v in dats.items():
            dats[k] = v.loc[:,v.columns.to_series().isin(bins.keys())]
        if no_cores is None:
            no_cores = os.cpu_count()-1
        start=0
        ...
        for k, v in tmp.items():
            woe_dat = pd.concat(v,axis=1)
            woe_dat = woe_dat.rename(columns=woe_dat.columns.to_series().apply(lambda x:x.replace('_woe','')))
            woe_dats[k] = woe_dat
        return woe_dats
